...
export class FunctionTool extends BaseTool {
   ...
   */
  private prepareFunctionArguments(args: Record<string, any>, context: ToolContext): any[] {
    ...
    for (const paramName of paramNames) {
      if (paramName === 'toolContext' || paramName === 'context') {
        ...
        if (isRequired) {
          throw new ToolError(`Missing required parameter: ${paramName}`);
        }
   ...
   */
  private getFunctionParameterNames(): string[] {
...
export function createTool(
  func: Function,
  config?: Partial<FunctionToolConfig>
): FunctionTool {
...
export function tool(
  config?: Partial<Omit<FunctionToolConfig, 'func'>>
) {
