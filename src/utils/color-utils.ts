...
export class ColorUtils {
   ...
   * Convert RGB to hex
   */
  static rgbToHex(rgb: RgbColor, includeAlpha = false): string {
   ...
   */
  static meetsContrastRequirement(foreground: string | RgbColor, background: string | RgbColor, level: 'AA' | 'AAA' = 'AA', size: 'normal' | 'large' = 'normal'): boolean {
   ...
   */
  static generatePalette(baseColor: string, count = 5): string[] {
   ...
   */
  static random(options: { hue?: number; saturation?: number; lightness?: number; alpha?: number } = {}): string {
    const {
    ...
    const hsl: HslColor = { h: hue, s: saturation, l: lightness, a: alpha };
   ...
   */
  static toCss(color: string | RgbColor | HslColor, format: 'hex' | 'rgb' | 'hsl' = 'hex'): string {
    let rgb: RgbColor;
   ...
   */
  static getAccessibleTextColor(backgroundColor: string, options: { level?: 'AA' | 'AAA'; preferDark?: boolean } = {}): string {
